...
### Initial removal as filebot integration is not expected to be required. May re-add if required.
...
VIDEO_EXTS          = [ 'mp4' ] # Replaced with only output coming from TubeArchivist at this point in time.
...
TA_CONFIG = ConfigType = get_ta_config()


class TubeArchivist:
    """connect to Tube Archivist"""

    ta_token: str = TA_CONFIG["ta_token"]
    headers: dict = {"Authorization": "Token {}".format(ta_token)}
    base: str = TA_CONFIG["ta_url"] if TA_CONFIG["ta_url"] else TUBEARCHIVIST_URL_BASE
    ...
    def get(self, path: str) -> TAVideo:
        ...
        raise ValueError("Video not found in TA instance: {}".format(path))
...
def env_check() -> None:
    ...
    if not os.path.exists(TA_CONFIG["ta_video_path"]):
        raise FileNotFoundError("Failed to access ta_video_path: {}".format(TA_CONFIG["ta_video_path"]))
    if not os.path.exists(TA_CONFIG["ta_cache_path"]):
        raise FileNotFoundError("Failed to access ta_cache_path: {}".format(TA_CONFIG["ta_cache_path"]))
    

def clean_overview(overview_raw: str) -> str:
    """parse and clean raw overview text"""
    if len(overview_raw) > 500:
        overview_raw = overview_raw[:496] + " ..."
...
if __name__ == '__main__':  #command line
  print("TA Series Scanner by lamusmaser for TubeArchivist")
  path  = sys.argv[1]
